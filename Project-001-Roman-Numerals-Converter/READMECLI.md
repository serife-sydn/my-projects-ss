...
aws ec2 create-security-group \
    --group-name roman_numbers_sec_grp_serf\
    --description "This Seec Group is to allow ssh and http from anywhere"
```
...
```bash
aws ec2 run-instances --image-id $LATEST_AMI --count 1 --instance-type t2.micro --key-name first-key-pair --security-group-ids sg-0bbceec0ed1577a73 --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=roman_numbers}]' --user-data file:///home/ec2-user/userdata.sh
or
...
```bash
aws ec2 terminate-instances --instance-ids i-0851182c6fcb0c16d ( buraya instance id yazılır)
```
